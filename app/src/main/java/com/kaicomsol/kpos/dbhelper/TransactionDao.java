...
public interface TransactionDao {
    ...
    @Query("SELECT * from `Transaction` ORDER BY cardIdm ASC")
    LiveData<List<Transaction>> getAllTransaction();

    @Query("SELECT * from `Transaction` WHERE cardIdm = :cardIdm")
    LiveData<Transaction> getTransactionByCardIdm(String cardIdm);

    // We do not need a conflict strategy, because the transaction is our primary key, and you cannot
