...
//             }
            ...
            // parse the results textdata only get
            //return  parse ( res );
            return  null;
        }  catch  ( Exception  e )  {
    ...
    private  byte []  readWithoutEncryption ( byte []  idm ,  int  size ,  byte []  serviceCode )  throws IOException {
        ...
        bout . write ( 0 );               // textdata length byte dummy
        ...
        // Because the specification of the refund code is little endian, it specifies from the low order byte.
        bout . write ( serviceCode [ 1 ]);  // refund code lower byte
        bout . write ( serviceCode [ 0 ]);  // refund code high byte
